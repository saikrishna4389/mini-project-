...
def clean_list(x):
    if isinstance(x, pd.Series):
        ...
        x = [i for i in x if pd.notnull(i)]
    result = []
    for i in x:
        if isinstance(i, str) and i.startswith('['):
            try:
                parsed = ast.literal_eval(i)
                result.extend([str(p).strip() for p in parsed])
            except:
                result.append(i)
        else:
...
# Get extra info: description, medications, etc.
def helper(dis):
    ...
    med = clean_list(medications[medications['Disease'] == dis]['Medication'].values)
